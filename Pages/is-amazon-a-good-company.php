  ...
  <div class="jumbo-container">
    <div class="row">
      ...
      <div class="col-sm-4 sides-containers" id="side-2-container">
		...
		<div class="side-2-comment" id="sides">
			<h1 style="color: hsl(45, 100%, 50%)"><?php echo $Query[0][6]; ?></h1>
	...
	<div class="img-info-container">
		<div class="col-sm-6 img-container">
			<img id="debate-img" src="CommentImages/<?php echo $Query[0][10]; ?>" alt="">
			<h5 id="img-subtext"><?php echo $Query[0][10]; ?><h5>
		</div>
		<div class="col-sm-6 info-container">
			<h4 id="debate-info"><?php echo $Query[0][7]; ?></h4>
		</div>
	</div>
  </div>
</div>
...
echo "<p>You don't seem to be signed in!<a href="Registration.php"> You can do that here</a>";
 ...
 $StatementQueries = $pdo->query("SELECT `CONTENT` FROM `post` WHERE thread_id = $ThisPageID  ORDER BY RAND() LIMIT 5;");
  $NumComments = $pdo->query("SELECT `CONTENT` FROM `post` WHERE thread_id = $ThisPageID  ORDER BY RAND() LIMIT 5;") -> fetchColumn();
 $Return = $StatementQueries->fetchAll();
 ...
 echo   '<div class="posted-comments">
	<p class="comment-text" id="top-comment">' . $StatementQueries[$i][0] . '<p>	
  </div>
</div>';
 }
 }
