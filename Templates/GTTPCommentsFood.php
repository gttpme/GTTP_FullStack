    ...
    <div class="row">
      ...
      <div class="col-sm-4 sides-containers" id="side-2-container">
		...
		<div class="side-2-comment" id="sides">
			<h1 style="color: hsl(100, 80%, 75%)"><?php echo $Query[0][6]; ?></h1>
...
echo "<p>You don't seem to be signed in!<a href="Registration.php"> You can do that here</a>";
 ...
 $StatementQueries = $pdo->query("SELECT `CONTENT`,`id` FROM `post` WHERE thread_id = $ThisPageID  ORDER BY RAND() LIMIT 5;");
 ...
 echo   '<div class="posted-comments">
	<p class="comment-text" id="top-comment"><span class="UpvoteIcon"> <i class="fa fa-arrow-circle-up" id=' .$UserComments[$i][1] .' ></i></span> ' . $UserComments[$i][0] . '<p>	
  </div>
</div>';
 }
 }
...
.side-1-comment {
background-color:hsl(100, 80%, 75%) ;
border-radius: 15px;
}
